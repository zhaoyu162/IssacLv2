...
namespace DZHClient
{
    class UDPClient
    {
        internal UDPClient(Dictionary<string, StockEntity> dictRef)
        {
            _dtScrach = _dtScrach.AddHours(8);
            _dictSERef = (dictRef);
            _ipEndPoint = new IPEndPoint(IPAddress.Loopback, 12425);
            ...
            _udpClient.Client.ReceiveBufferSize = 1024 * 1024 * 100;
        ...
        void _ResolveQQData(BinaryReader br)
        {
            ...
            for (var n = 0; n < count; n++)
            {
                ...
                var code = Encoding.ASCII.GetString(br.ReadBytes(12)).ToLower().Trim('\0');
                var name = Encoding.GetEncoding(936).GetString(br.ReadBytes(32)).ToLower().Trim('\0');
        ...
        void _ResolveBigorder(BinaryReader br)
        {
            ...
            var code = Encoding.ASCII.GetString(br.ReadBytes(16)).ToLower().Trim('\0');
            var name = Encoding.GetEncoding(936).GetString(br.ReadBytes(32)).ToLower().Trim('\0');
        ...
        void _ResolveL1Quote(BinaryReader br)
        {
            ...
            {
                var code = Encoding.ASCII.GetString(br.ReadBytes(16)).ToLower().Substring(1).Trim('\0');
                if (code.Length == 0)
                {
                    br.ReadBytes(170 - 16);
                    break ;
                }
                var name = Encoding.GetEncoding(936).GetString(br.ReadBytes(16)).Trim('\0');
